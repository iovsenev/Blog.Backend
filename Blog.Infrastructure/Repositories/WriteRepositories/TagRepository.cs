...
public class TagRepository : ITagRepository
{
    ...
    public async Task<Result<TagEntity>> GetByTagNameAsync(string tagName, CancellationToken ct)
    {
        var tagResult = await _context.Tags.FirstOrDefaultAsync(t => t.TagName == tagName, ct);

        return tagResult is null ? Error.NotFound($"Entity with {tagName} not found.") : tagResult;
    }
}
